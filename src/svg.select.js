function getMoseDownFunc (eventName, el) {
  return function (ev) {
    ...
    console.log(eventName, { x: x, y: y, event: ev })
    el.fire(eventName, { x: x, y: y, event: ev })
  }
}

export default function allowSelection (svgJs) {
  const { Element, extend, G, Point } = svgJs
  ...
  class SelectHandler {
    ...
    // First transform all points, then draw polygon out of it
      this.selection.polygon(this.points.slice(0, this.order.indexOf('rot')).map(el => [el.x, el.y])).addClass('selection_border').fill('none')
    }
    ...
    createResizeHandles () {
      this.points.slice(0, this.order.indexOf('rot')).forEach((p, index) => {
        ...
        console.log(this.order[index], this.el)
      })
    }
    ...
    updatePoints () {
      ...
      this.points.map((p, i) => console.log(i === 0 && p))
    }
